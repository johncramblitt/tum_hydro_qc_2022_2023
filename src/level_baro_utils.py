...
def read_normalize_solinst_data(fn, header_num, sensor_type):
    ...
    if sensor_type == 'lvl':
        ...
        df['LEVEL'] *= KPA_TO_CM
        #choose based on barologger header
...
def plot_in_water(df, insert_datetime_str, remove_datetime_str, site_info, sensor_type='lvl', figsize=config.FIGSIZE):
    ...
    # ** INSERT **
    if len(insert_datetime_str)<=16:
        insert_datetime = datetime.strptime(insert_datetime_str, '%Y-%m-%d %H:%M')
    if len(insert_datetime_str)>16:
        insert_datetime = datetime.strptime(insert_datetime_str, '%Y-%m-%d %H:%M:%S')
    ...
    if len(remove_datetime_str)<=16:
        remove_datetime = datetime.strptime(remove_datetime_str, '%Y-%m-%d %H:%M')
    if len(remove_datetime_str)>16:
        remove_datetime = datetime.strptime(remove_datetime_str, '%Y-%m-%d %H:%M:%S')
...
def plot_candidate(ds, candidate_bounds, flag_bounds, site_info, figsize=config.FIGSIZE):
    fig, ax = plt.subplots(nrows=1, ncols=3, figsize=figsize)
    plt.suptitle(f'{site_info["site"]} {site_info["span"]}')
    
    ax[0].plot(ds)
    ax[0].plot(ds[candidate_bounds['start']: candidate_bounds['end']], label='Candidate')
    ax[0].axhline(y=(config.NO_FLOW_LEVEL[site_info['sitecode']]*100),linestyle='-.',c='gray',label='Level of no flow')
    ax[0].legend()
    ax[0].set_ylabel('cm')
    ...
    if flag_bounds == None:
        ...
        ax[1].plot(ds.loc[(ds.index>=pd.to_datetime(candidate_bounds['start']))&(ds.index<pd.to_datetime(flag_bounds['start']))],color='C0')
        ax[1].plot(ds.loc[(ds.index>pd.to_datetime(flag_bounds['end']))&(ds.index<=pd.to_datetime(candidate_bounds['end']))],color='C0')
        flag_start_loc = ds.index.get_loc(pd.to_datetime(flag_bounds['start']))
        flag_end_loc = ds.index.get_loc(pd.to_datetime(flag_bounds['end']))
        ax[1].plot(ds[flag_start_loc-1: flag_end_loc+2],c='firebrick',label='Flag')
        ax[1].legend()
    ax[1].set_ylabel('cm')
    ...
    if flag_bounds == None:
        ax[2].plot(ds[candidate_bounds['start']: candidate_bounds['end']])
        ax[2].scatter(ds[candidate_bounds['start']: candidate_bounds['end']].index,ds[candidate_bounds['start']: candidate_bounds['end']],c='navy',s=10)
    else: 
        ax[2].plot(ds.loc[(ds.index>=(pd.to_datetime(flag_bounds['start'])-timedelta(0.2)))&(ds.index<(pd.to_datetime(flag_bounds['start'])))],color='C0')
        ax[2].plot(ds.loc[(ds.index>pd.to_datetime(flag_bounds['end']))&(ds.index<=(pd.to_datetime(flag_bounds['end'])+timedelta(0.2)))],color='C0')
        ax[2].plot(ds[flag_start_loc-1: flag_end_loc+2],c='firebrick',label='Flag')
        ax[2].scatter(ds[(pd.to_datetime(flag_bounds['start'])-timedelta(0.2)): (pd.to_datetime(flag_bounds['end'])+timedelta(0.2))].index,ds[(pd.to_datetime(flag_bounds['start'])-timedelta(0.2)): (pd.to_datetime(flag_bounds['end'])+timedelta(0.2))],c='navy',s=10)
        ax[2].scatter(ds[flag_bounds['start']: flag_bounds['end']].index,ds[flag_bounds['start']: flag_bounds['end']],c='maroon',s=10)
        ax[2].legend()
    ax[2].set_ylabel('cm')
    ax[2].set_title('Profile of Ice Jam/Displacement Candidate')
    ax[2].tick_params('x', labelrotation=45)

def plot_candidate_temp(ds, candidate_bounds, flag_bounds, site_info, data_range, figsize=config.FIGSIZE):
    ...
    ax[0].set_title('Air Temperature During Candidate Relative to Whole Series')
    ax[0].set_xlim(data_range)
    ax[0].tick_params('x', labelrotation=45)
    
    ax[1].plot(ds[candidate_bounds['start']: candidate_bounds['end']])
    ax[1].set_ylabel('degrees C')
    ax[1].set_title('Air Temperature Profile of Candidate')
    ax[1].tick_params('x', labelrotation=45)

def plot_candidate_prate(ds, candidate_bounds, flag_bounds, site_info, data_range, figsize=config.FIGSIZE):
    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=figsize)
    plt.suptitle(f'{site_info["site"]} {site_info["span"]}')
    ...
    ax[0].plot(ds[candidate_bounds['start']: candidate_bounds['end']], label='Candidate')
    ax[0].legend()    
    ax[0].set_ylabel('kg/m^2/s')
    ax[0].set_title('Precipitation Rate During Candidate Relative to Whole Series')
    ax[0].set_xlim(data_range)
    ax[0].tick_params('x', labelrotation=45)
    
    ax[1].plot(ds[candidate_bounds['start']: candidate_bounds['end']])
    ax[1].set_ylabel('kg/m^2/s')
    ax[1].set_title('Precipitation Rate Profile of Candidate')
    ax[1].tick_params('x', labelrotation=45)
    
def make_output_df(datetime_index, raw_pressure, barocorrected_pressure, temp, unknown_anomalies, ice_jams, variable_gauge_pools,sensor_malfunctions=None):
    ...
    if unknown_anomalies is not None:
        for unknown_anomaly in unknown_anomalies:
            output_df.loc[output_df[unknown_anomaly['start']:unknown_anomaly['end']].index, labels[-1]] = config.ANOMALY_FLAG

    if variable_gauge_pools is not None:
        for variable_gauge_pools in variable_gauge_pools:
            output_df.loc[output_df[variable_gauge_pools['start']:variable_gauge_pools['end']].index, labels[-1]] = config.VARIABLE_GAUGE_POOL_FLAG
...
def get_summary(level_path, 
    ...
    anom = 'Unknown Anomalies:'
    if ice_jams is None:
        anom += ' None\n'
    else:
        for i, ij in enumerate(unknown_anomalies, 1):
            anom += f'\nUnknown Anomaly {i}\nstart: {ij["start"]}\nend: {ij["end"]}\n\n'

    gauge = 'Variable Gauge Pools:'
    if variable_gauge_pools is None:
        gauge += ' None\n'
    else:
        for i, ij in enumerate(variable_gauge_pools, 1):
            gauge += f'\nVariable Gauge Pool {i}\nstart: {ij["start"]}\nend: {ij["end"]}\n\n'
    ...
    malfunc = 'Sensor Malfunctions:'
    if sensor_malfunctions is None:
        malfunc += ' None\n'
    else:
        for i, ij in enumerate(sensor_malfunctions, 1):
            malfunc += f'\nSensor malfunctions {i}\nstart: {ij["start"]}\nend: {ij["end"]}\n\n'

def resample_vented(vented_df):
    return vented_df[vented_df.index.minute % 30 == 0]

def make_vented_output_df(datetime_index, lvl, temp, cond, unknown_anomalies, ice_jams, variable_gauge_pools, sensor_malfunctions=None):
    if cond is None:
        data = [datetime_index, lvl, temp, np.zeros(len(lvl), dtype=np.int8)]
        labels = config.VENTED_CORRECTED_HEADER[0:3]+config.VENTED_CORRECTED_HEADER[4:]

    else: 
        data = [datetime_index, lvl, temp, cond, np.zeros(len(lvl), dtype=np.int8)]
    ...
    if variable_gauge_pools is not None:
        for vgp in variable_gauge_pools:
            output_df.loc[output_df[vgp['start']:vgp['end']].index, labels[-1]] = config.VARIABLE_GAUGE_POOL_FLAG
...
def read_vented_segments(sitecode, end_year):
    vented_segments = {}

    vented_segments[end_year]=[]

    year_dir = os.path.join('..', '..', f'vented_{end_year}')
    vented_dir_path = os.path.join(year_dir, 'data', 'processed')
    for fn in os.listdir(vented_dir_path):
        if config.SITE_SHORTNAME[sitecode] in fn:
            ...
            vented_segments[end_year].append(pd.read_csv(data_path, index_col=0, parse_dates=[0])) #infer_datetime_format=True))
...
def read_unvented_segments(sitecode, end_year):
    unvented_segments = {}

    unvented_segments[end_year] = []
    year_dir = os.path.join('..', '..', f'unvented_{end_year}')
    unvented_data_dir = os.path.join(year_dir, 'data', 'barocorrected')
    for fn in sorted(os.listdir(unvented_data_dir)):
        if config.SITE_SHORTNAME[sitecode] in fn:
            data_path = os.path.join(unvented_data_dir, fn)
            print(f'loading {data_path}')
            unvented_segments[end_year].append(pd.read_csv(data_path, 
                                                           index_col=0, 
                                                           parse_dates=[0], 
                                                           # infer_datetime_format=True
                                                           ))
...
def plot_all(title=None, prev=None, prev_weeks=24, stage=None, vented_segments=None, unvented_segments=None, vented_offsets=None, unvented_offsets=None, figsize=None):
    ...
    if vented_segments is not None:
        for year in vented_segments:
            vented_segment_offsets = []
            if vented_offsets is None:
                vented_segment_offsets = np.zeros(len(vented_segments[year]))
            else:
                vented_segment_offsets = vented_offsets[year]
            for i, (segment, offset) in enumerate(zip(vented_segments[year], vented_segment_offsets)):
                ax.plot(segment['vented_pressure(cm)'] + offset, label=f'vented {year}, segment {i}')
    ...
    if stage is not None:
        ax.scatter(stage.index, stage['stage(cm)'], label='stage', color='k',zorder=1)
...
def initialize_offsets(vented_segments, unvented_segments):
    ...
    vented_offsets = {}
    for year in vented_segments:
        vented_offsets[year] = np.zeros(len(vented_segments[year]))
...
def plot_discharge(stitched_df, sitecode):
    ...
    plt.plot(stitched_df[stitched_df['discharge_flag'] == 2]['estimated_discharge(cms)'], label='Ice Jam',marker='o')
    plt.plot(stitched_df[stitched_df['discharge_flag'] == 3]['estimated_discharge(cms)'], label='Malfunction',marker='o')
    plt.plot(stitched_df[stitched_df['discharge_flag'] == 1]['estimated_discharge(cms)'], label='Unknown Anomaly',marker='o')
    plt.plot(stitched_df[stitched_df['discharge_flag'] == 4]['estimated_discharge(cms)'], label='Variable Gauge Pool',marker='o')
    plt.xticks(rotation=30)
    plt.ylabel('cms')
...
def save_final_data(df, sitecode, startyear, endyear):
    ...
    output_dir = os.path.join('..','output','processed_data')
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
